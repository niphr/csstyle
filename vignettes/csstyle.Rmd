...
`csstyle` standardizes graphs, tables, and reports to follow the Core Surveillance visual guidelines. The design is deliberately narrow. The package does not expose every ggplot2 option. It offers a small set of outputs that look the same, whoever produces them and whenever.
...
`theme_cs()` is the main theme function. It applies the Core Surveillance appearance to any ggplot2 plot.
...
Every number formatting function belongs to one of two parallel families. The two families reflect the intended audience. The Norwegian family follows domestic conventions. The journal family follows international conventions for academic publications. The "Journal formatting" section below compares the two side by side.
...
For academic publications the package provides a parallel set of functions. They use a decimal point, a comma thousands separator, `"NA"` for missing values, and ISO 8601 dates.
...
Both families include inverse log transformation variants. Use them when the axis values are on a log scale and you want the labels to show the original units.
...
Run `help(package = "csstyle")` for the full function list. Individual help pages such as `?theme_cs`, `?scale_color_cs` and `?format_num_as_nor_num_1` document single functions.